...
const ProfilePage = () => {
  ...
  return (
    <div className={styles.container}>
      <div className={styles.title1}>Katıldığınız Testler</div>
      <div className={styles.testsContainer}>
        {isNull && <div>Herhangi bir teste katılmadınız</div>}
        {!isNull &&
          tests.map((item) => {
            ...
            return (
              <div className={styles.userDetailContainer} key={test.TestId}>
                <div style={{width:"20%"}}>{item.TestName}</div>
                <div>
                  <div style={{ color: "#089404" }}>
                    Doğru: {counts.truecount}
                  </div>
                  <div style={{ color: "#8B0000" }}>
                    Yanlış: {counts.wrongCount}
                  </div>
                  <div style={{ color: "#9ECAEF" }}>
                ...
                {/* <div className={styles.cubeContainer} style={{width:"150px"}}>
                  {test.AnswerList.map((item, index) => {
                    if (item.Answer == "e") {
                      ...
                      if (item.Result) {
                        return (
                          <div className={styles.trueCube}>{index + 1}</div>
                        );
                      } else {
                        return (
                          <div className={styles.wrongCube}>{index + 1}</div>
                        );
                      }
                    }
                  })}
